...
namespace libdebug
{
    public partial class PS5DBG
    {
        ...
        private const int PS5DBG_PORT = 744;
        private const int PS5DBG_DEBUG_PORT = 755;
        private const int PS5DBG_KLOG_PORT = 3232;
        ...
        //    uint8_t original;
        //};
        public static uint MAX_BREAKPOINTS = 30; // ps5debug-NG supports 30 software breakpoint slots (vs 10 on ps4debug)
        ...
        public enum CMDS : uint
        {
            ...
            // ps5debug-NG only: no status word precedes these responses (see GetConsoleFirmwareVersion / GetConsoleBranding)
...
//                            bytes = BitConverter.GetBytes(b);
        ...
        {
            if (!IsConnected)
                throw new Exception("libdbg5: not connected");
        }

        private void CheckDebugging()
        {
            if (!IsDebugging)
                throw new Exception("libdbg5: not debugging");
        }

        /// <summary>
        /// Initializes PS5DBG class
        /// </summary>
        /// <param name="addr">PlayStation 5 address</param>
        public PS5DBG(IPAddress addr)
        {
            enp = new IPEndPoint(addr, PS5DBG_PORT);
        ...
        /// Initializes PS5DBG class
        /// </summary>
        /// <param name="ip">PlayStation 5 ip address</param>
        ...
        /// <summary>
        /// Disconnects from PlayStation 5
        /// </summary>
        ...
        /// <summary>
        /// Get current ps5debug-NG client library version
        /// </summary>
        ...
        /// <summary>
        /// Get the current ps5debug-NG protocol version from console
        /// </summary>
        ...
        /// <summary>
        /// Get the running console firmware version as a decimal number (e.g. 900 = 9.00, 1240 = 12.40).
        /// ps5debug-NG only; no status word precedes the response.
        /// </summary>
        public ushort GetConsoleFirmwareVersion()
        {
            CheckConnected();

            SendCMDPacket(CMDS.CMD_FW_VERSION, 0);
        ...
        /// <summary>
        /// Get the server branding string, e.g. "ps5debug-NG by OSR v1.3.2 [fingerprint]",
        /// plus the capability level appended after a NUL. ps5debug-NG only; no status word precedes the response.
        /// </summary>
        public string GetConsoleBranding()
        {
            CheckConnected();

            SendCMDPacket(CMDS.CMD_BRANDING, 0);
            ...
            // bytes are: human brand string, NUL, capability level (no trailing NUL) - return the brand portion
            return ConvertASCII(data, 0);
        }
    }
}